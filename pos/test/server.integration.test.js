...
test('order:delete は番号が使い回されていても、uidで指定した特定の注文だけを削除する', async () => {
  const { httpServer, close } = buildServer({
    dataFile: tempDataFile(),
    now: () => new Date('2026-10-16T10:00:00+09:00'),
  });
  ...
  async function checkoutReadyHanded() {
    client.emit('cart:addItem', { product: 'hotdog', toppings: { ketchup: 'normal', mustard: 'normal', mayo: 'normal' } });
    await waitForState(client);
    client.emit('cart:setReceived', { amount: 200 });
    await waitForState(client);
    const checkoutResult = await new Promise((resolve) => client.emit('order:checkout', {}, resolve));
    const { day, id } = checkoutResult.order;
    await new Promise((resolve) => client.emit('order:ready', { day, id }, resolve));
    const handedResult = await new Promise((resolve) => client.emit('order:handed', { day, id }, resolve));
    return handedResult.order;
  }

  const first = await checkoutReadyHanded();
  const second = await checkoutReadyHanded();
  assert.equal(second.id, first.id, '番号が使い回されている前提');

  const stateAfterDelete = waitForState(client);
  const deleteResult = await new Promise((resolve) => client.emit('order:delete', { uid: first.uid }, resolve));
  assert.equal(deleteResult.ok, true);
  const state = await stateAfterDelete;

  const firstOrder = state.orders.find((o) => o.uid === first.uid);
  const secondOrder = state.orders.find((o) => o.uid === second.uid);
