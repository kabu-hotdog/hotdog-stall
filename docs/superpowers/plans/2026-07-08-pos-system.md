...
test('会計するとchecktoutが番号1を返し、全クライアントにブロードキャストされる', async () => {
  ...
  const posClient = await connectClient(port);
  const kitchenClient = await connectClient(port);
  await waitForState(posClient);
...
test('調理完了→受渡済のフローが全クライアントに反映される', async () => {
  ...
  const posClient = await connectClient(port);
  const smartboardClient = await connectClient(port);
  await waitForState(posClient);
