...
- 現在の預かり金の値（`latestState.cart.received`、既存のグローバル変数）を文字列として扱い、末尾に桁を足した上で数値に変換してサーバーへ送る。
...
- 数字ボタン（`.key[data-digit]`）のクリックハンドラは、現在の預かり金（`latestState.cart.received`）を文字列化して末尾にボタンの`data-digit`値を連結し、数値に変換して `socket.emit('cart:setReceived', { amount })` を呼ぶ
  - 例: 現在500円のとき「1」ボタン → `String(500) + '1'` → `'5001'` → `Number('5001')` → `5001`
  - 「00」ボタンも同じ仕組みで`data-digit="00"`を連結するだけで良い（追加ロジック不要）
- 「クリア」ボタン（`#keyClear`）は `socket.emit('cart:setReceived', { amount: 0 })` を呼ぶ
